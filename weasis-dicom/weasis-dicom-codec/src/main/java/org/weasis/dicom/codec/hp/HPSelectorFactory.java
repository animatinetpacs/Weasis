...
import org.weasis.dicom.codec.hp.filter.FilterOp;
import org.weasis.dicom.codec.hp.filter.HPFilter;
import org.weasis.dicom.codec.hp.plugins.ImagePlaneSelector;
import org.weasis.dicom.codec.hp.spi.HPComparatorCategoryService;
import org.weasis.dicom.codec.hp.spi.HPSelectorCategoryService;
...
public class HPSelectorFactory {
  ...
  private static HPSelector createAttributeValueSelector(
    ...
    return switch (CodeString.getVR(vrStr)) {
      case VR.AT -> new IntSelector(item, Tag.SelectorATValue, match, filterOp, VR.AT);
      case VR.CS -> new Str(item, Tag.SelectorCSValue, match, filterOp, VR.CS);
      case VR.DS -> new Flt(item, Tag.SelectorDSValue, match, filterOp, VR.DS);
      case VR.FD -> new Dbl(item, Tag.SelectorFDValue, match, filterOp, VR.FD);
      case VR.FL -> new Flt(item, Tag.SelectorFLValue, match, filterOp, VR.FL);
      case VR.IS -> new IntSelector(item, Tag.SelectorISValue, match, filterOp, VR.IS);
      case VR.LO -> new Str(item, Tag.SelectorLOValue, match, filterOp, VR.LO);
      case VR.LT -> new Str(item, Tag.SelectorLTValue, match, filterOp, VR.LT);
      case VR.PN -> new Str(item, Tag.SelectorPNValue, match, filterOp, VR.PN);
      case VR.SH -> new Str(item, Tag.SelectorSHValue, match, filterOp, VR.SH);
      case VR.SL -> new IntSelector(item, Tag.SelectorSLValue, match, filterOp, VR.SL);
      case VR.SQ -> new CodeValueSelector(item, match, filterOp, VR.SQ);
      case VR.SS -> new IntSelector(item, Tag.SelectorSSValue, match, filterOp, VR.SS);
      case VR.ST -> new Str(item, Tag.SelectorSTValue, match, filterOp, VR.ST);
      case VR.UL -> new UIntSelector(item, Tag.SelectorULValue, match, filterOp, VR.UL);
      case VR.US -> new IntSelector(item, Tag.SelectorUSValue, match, filterOp, VR.US);
      case VR.UT -> new Str(item, Tag.SelectorUTValue, match, filterOp, VR.UT);
  ...
  private static HPSelector createAttributeValueSelector(
    ...
    int valueTag =
        switch (vr1) {
          case VR.CS -> Tag.SelectorCSValue;
          case VR.LO -> Tag.SelectorLOValue;
          case VR.LT -> Tag.SelectorLTValue;
          case VR.PN -> Tag.SelectorPNValue;
          case VR.SH -> Tag.SelectorSHValue;
          case VR.ST -> Tag.SelectorSTValue;
          case VR.UT -> Tag.SelectorUTValue;
  ...
  private static HPSelector createAttributeValueSelector(
    ...
    int valueTag =
        switch (vr1) {
          case VR.DS -> Tag.SelectorDSValue;
          case VR.FL -> Tag.SelectorFLValue;
          default -> throw new IllegalArgumentException("vr: " + vr);
        };
