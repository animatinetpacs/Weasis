 ...
 */
package org.weasis.dicom.codec.hp.plugins;
...
import org.weasis.dicom.codec.hp.AbstractHPSelector;
...
public class ImagePlaneSelector extends AbstractHPSelector {
  ...
  public boolean matches(Attributes attributes, int frame) {
    ...
    if (floats != null && floats.length == 6) {
      Vector3d vr = new Vector3d(floats);
      Vector3d vc = new Vector3d(floats[3], floats[4], floats[5]);
      imagePlane = ImageOrientation.getPlan(vr, vc, minCosine);
    } else {
      String[] ss = attributes.getStrings(Tag.PatientOrientation);
      if (ss != null && ss.length == 2) {
        Orientation rowAxis = Biped.getBiped(ss[0]);
        Orientation colAxis = Biped.getBiped(ss[1]);
